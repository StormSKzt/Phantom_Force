...
local esp = {} do
    ...
    esp_settings.teamcolor = Color3.fromRGB(0, 255, 255) -- 121,255,97, 57,255,20
    esp_settings.enemycolor = Color3.fromRGB(255, 0, 0) -- 238,38,37, 255,0,13, 255,7,58
...
local createwindow = loadstring(game:HttpGet("https://cdn.simplehub.store/UI-LIB/StrikeHub-Project1.lua"))().new()
local tap1 = createwindow:Tap("Main")
local tap2 = createwindow:Tap("Character")
local tap3 = createwindow:Tap("Visuals")
local tap4 = createwindow:Tap("Misc")

local page1 = tap1:page("Slient Aim")
local page2 = tap1:page("FOV")

local page3 = tap2:page("Character Settings")
local page4 = tap2:page("Character Settings")

local page5 = tap3:page("Visuals")
local page6 = tap3:page("ESP")

local page7 = tap4:page("Misc Tab")

page1:Toggle("Slient Aim",shais.silentaim,function(new)
    shais.silentaim = new
end)

local headshotchance = page1:Slider("Headshot Chance",0,100,shais.headshotchance,1,function(new)
    shais.headshotchance = new
end)
----------------------------------------------------------------------
page2:Toggle("Enable FOV",shais.fovcircle,function(new)
    shais.fovcircle = new
end)

local headshotchance = page2:Slider("Radius",0,1000,shais.fov,1,function(new)
    shais.fov = new
end)

local sides = page2:Slider("FOV Style",0,40,shais.fovsides,1,function(new)
    shais.fovsides = new
end)

local thickness = page2:Slider("Thickness",1,50,shais.fovthickness,1,function(new)
    shais.fovthickness = new
end)
-------------------------------------------------------------------------
page3:Toggle("No fall damage",shais.nofalldamage,function(value)
    shais.nofalldamage = value
end)
page3:Toggle("Enable WalkSpeed",shais.setwalkspeed,function(new)
...
page3:Toggle("Enable JumpPower",shais.setjumppower,function(new)
    shais.setjumppower = new
end)
-------------------------------------------------------------------------
local walkspeed = page4:Slider("Walk Speed",1,100,shais.walkspeed,1,function(new)
    shais.walkspeed = new
end)
local jumppower = page4:Slider("Jump Power",1,50,shais.jumppower,1,function(new)
    shais.jumppower = new
end)
-----------------------------------------------------------------------
page5:Toggle("Color Effect",shais.colorEffect,function(new)
    shais.colorEffect = new
end)
page5:Toggle("Gun visuals",shais.gunVisuals,function(new)
    shais.gunVisuals = new
end)
page5:Toggle("Dead Visuals",shais.deadVisuals,function(new)
    shais.deadVisuals = new
end)
page6:Toggle("ESP Players",esp.enabled,function(new)
    esp.enabled = new
end)
page6:Toggle("ESP Teammate",esp.showteam,function(new)
    esp.showteam = new
end)
------------------------------------------------------------------
page7:Toggle("Auto Rejoin",true,function(value)
    _G.AutoRejoin = value
end)
spawn(function()
    while wait() do
...
page7:Button("Re join", function()
    local ts = game:GetService("TeleportService")
    local p = game:GetService("Players").LocalPlayer
    ts:Teleport(game.PlaceId, p)
end)

page7:Button("Server Hop", function()
